...
@Slf4j
@RequestMapping("/api")
public class AdministrativeDivisionsController {
    private final AdministrativeDivisionService service;
    private final AdministrativeDivisionDtoMapper mapper;

    @GetMapping("/administrative-divisions/{id}")
    @CrossOrigin(origins = "*")
    public ResponseEntity<ControllerResult<AdministrativeDivisionDto>> findById(@PathVariable Long id) {
    ...
    @CrossOrigin(origins = "*")
    public ResponseEntity<ControllerResult<List<AdministrativeDivisionDto>>> findAll() {
